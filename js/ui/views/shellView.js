...
(function (App) {
  ...
  const ICONS = {
    ...
    audit: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 12h6M9 16h6M9 8h6M5 21h14a2 2 0 002-2V7l-5-5H5a2 2 0 00-2 2v15a2 2 0 002 2z"/></svg>',
    location: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 12-9 12s-9-5-9-12a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>',
  ...
  const NAV_BY_ROLE = {
    admin: [
      ...
      { path: '/ubicaciones', label: 'Ubicaciones', icon: ICONS.location },
      { path: '/reportes', label: 'Reportes', icon: ICONS.reports },
      { path: '/auditoria', label: 'Auditoría', icon: ICONS.audit },
  ...
  function doLogout(ev) {
    ...
    const outgoingSession = App.services.authService.getCurrentSession();
    App.services.authService.logout();
    bus.emit('auth:logout', outgoingSession);
  }
